...
if($request['method'] == "POST")
	...
	if($request['content-type'] == "application/json"){
		...
		if(!is_null($accessToken) && !is_null($clientToken)) {
			...
			$req = Core\Queries::execute('SELECT * FROM joueurs WHERE clientToken=:clientToken', ['clientToken' => $clientToken]);
			...
			if(!empty($req))
				...
				if($accessToken == $req->access_token)
					// Updating the access and the client token in the database
					Core\Queries::execute("UPDATE joueurs SET access_token=:accessToken WHERE clientToken=:clientToken", ['clientToken' => $clientToken, 'accessToken' => '']);
