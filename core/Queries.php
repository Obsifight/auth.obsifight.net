...
Class Queries{
     ...
     */
	public static function query($query, $db = 'default'){
		// Executing the query from the database instance
		if($db == 'default') {
			$results = Database::getInstance()->query($query);
		} elseif($db == 'log') {
			$results = Database::getLogInstance()->query($query);
		}
     ...
     */
	public static function execute($query, $params=[], $db = 'default'){
		// Preparing the query with the database instance
		if($db == 'default') {
			$results = Database::getInstance()->prepare($query);
		} elseif($db == 'log') {
			$results = Database::getLogInstance()->prepare($query);
		}
