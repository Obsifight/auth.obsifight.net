...
Class Database{
	...
	public static function getLogInstance() {
		// If the current database and the current config are null
		if(is_null(self::$db_log) && is_null(self::$config_log)) {
			// Getting the database config from the config file
			self::$config_log = Config::get('database_log');

			// Trying to create a \PDO object from the configuration
			try {
				$db_log = new \PDO('mysql:dbname='. self::$config_log['database'] .';host='. self::$config_log['host'] .'', self::$config_log['username'], self::$config_log['password']);
			} catch (PDOException $e) {
				// If it failed, printing the error message
			    echo $e->getMessage();
			}
